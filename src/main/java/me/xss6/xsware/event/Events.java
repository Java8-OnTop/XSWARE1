...
public class Events implements Globals {
    ...
    public void onAttackEntity(AttackEntityEvent event) {
        if (!nullCheck()) {
            XSWARE.KD_MANAGER.onAttackEntity(event);
        }
    }
    ...
    public void onSendAttackPacket(PacketEvent.Send event) {
        if (!nullCheck()) {
            XSWARE.KD_MANAGER.onSendAttackPacket(event);
        }
    }
    ...
    public void onEntityDeath(DeathEvent event) {
        if (!nullCheck()) {
            XSWARE.KD_MANAGER.onEntityDeath(event);
        }
    }
    @SubscribeEvent
    ...
    @CommitEvent(priority = me.xss6.xsware.event.processor.EventPriority.HIGH)
    public void onUpdateWalkingPlayerPost(UpdateWalkingPlayerEvent event) {
        if (nullCheck()) {
            return;
        }
        if (event.getStage() == 0) {
            XSWARE.ROTATION_MANAGER.updateRotations();
            XSWARE.POS_MANAGER.updatePosition();
        }
        if (event.getStage() == 1) {
            XSWARE.ROTATION_MANAGER.restoreRotations();
            XSWARE.POS_MANAGER.restorePosition();
        }
    }
    ...
    public void onWorldRender(RenderWorldLastEvent event) {
        ...
        mc.profiler.startSection("xsware");
    ...
    public void onChatSent(ClientChatEvent event) {
        if (event.getMessage().startsWith(XSWARE.COMMANDS.getPrefix())) {
            event.setCanceled(true);
            try {
                mc.ingameGUI.getChatGUI().addToSentMessages(event.getMessage());
                if (event.getMessage().length() > 1) {
                    XSWARE.COMMANDS.executeCommand(event.getMessage().substring(1));
                } else {
    ...
    public final void onPacketSend(@NotNull PacketEvent.Send event) {
        XSWARE.ROTATION_MANAGER.onPacketSend(event);
    }
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        XSWARE.SERVER_MANAGER.onPacketReceived();
        if (event.getPacket() instanceof SPacketPlayerPosLook) {
            XSWARE.ROTATION_MANAGER.onPacketReceive((SPacketPlayerPosLook) event.getPacket());
        }
        if (event.getPacket() instanceof SPacketEntityStatus) {
            ...
            try {
                if (packet.getOpCode() == 0x23 && packet.getEntity(mc.world) instanceof EntityPlayer) {
                    ...
                    XSWARE.EVENT_PROCESSOR.addEventListener(new TotemPopEvent(player));
                    Chams.INSTANCE.onPopLol(new TotemPopEvent(player));
                    XSWARE.POP_MANAGER.onTotemPop(player);
                }
            ...
            packet.getEntries().stream().filter(Objects::nonNull).filter(data -> !Strings.isNullOrEmpty(data.getProfile().getName()) || data.getProfile().getId() != null).forEach(data -> {
                ...
                switch (packet.getAction()) {
                    case ADD_PLAYER: {
                        ...
                        XSWARE.EVENT_PROCESSOR.addEventListener(new ConnectionEvent(0, id, name));
                        break;
                    }
                    case REMOVE_PLAYER: {
                        ...
                        if (entity != null) {
                            ...
                            XSWARE.EVENT_PROCESSOR.addEventListener(new ConnectionEvent(1, entity, id, logoutName));
                            break;
                        }
                        XSWARE.EVENT_PROCESSOR.addEventListener(new ConnectionEvent(2, id, null));
                    }
                    default:
                        break;
                }
