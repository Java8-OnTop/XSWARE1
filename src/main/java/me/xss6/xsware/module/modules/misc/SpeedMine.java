...
import me.xss6.xsware.event.events.BlockEvent;
...
@Module.Registration(name = "Speed Mine", description = "break shit fast idfk", category = Module.Category.MISC, priority = ModulePriority.Highest)
    ...
    public void onUpdate() {
        ...
        if(!isActive && (packetBurrow.getValue() || packetCity.getValue())){
            ...
            if(packetBurrow.getValue() && !isActive){
                for (EntityPlayer entity : mc.world.playerEntities.stream().filter(entityPlayer -> !XSWARE.FRIEND_MANAGER.isFriend(entityPlayer.getName())).collect(Collectors.toList()))
                {
