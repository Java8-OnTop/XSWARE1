...
@Module.Registration(name = "Crystal Trap", description = "Traps your enemy and proceeds to KILL him WTF?!?!?!", category = Module.Category.COMBAT, priority = ModulePriority.Highest)
public class CrystalTrap extends Module {
    ...
    private void check() {
        ...
        if (player.isDead) {
            ClientMessage.sendMessage("GOT KILLED EZZ");
            this.disable();
            return;
        }
        if (!EntityUtil.isInHole(player)) {
            ClientMessage.sendMessage("lol your target ran out of the hole");
            this.disable();
            return;
        }
        ...
        if (!(mc.world.getBlockState(player.getPosition().add(0, 3, 0)).getBlock() instanceof BlockAir) || !(mc.world.getBlockState(player.getPosition().add(0, 4, 0)).getBlock() instanceof BlockAir) || !(mc.world.getBlockState(player.getPosition().add(0, 5, 0)).getBlock() instanceof BlockAir)) {
            ClientMessage.sendMessage("no space here");
            this.disable();
            return;
        }
        crystal = getCrystal();
    ...
    public EntityPlayer findClosestTarget()  {
        ...
        for (final EntityPlayer target : mc.world.playerEntities)
        {
            ...
            if (XSWARE.FRIEND_MANAGER.isFriend(target.getName()))
                continue;
