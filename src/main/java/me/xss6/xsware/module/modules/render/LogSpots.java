...
import me.xss6.xsware.event.events.ConnectionEvent;
...
@Module.Registration(name = "Logout Spots", description = "shows ez logs", category = Module.Category.RENDER, isListening = false)
public class LogSpots extends Module {
        ...
        if (event.getStage() == 0) {
            ...
            if (name != null && uuid != null) {
                this.spots.add(new LogoutPos(name, uuid, entity, Math.round(entity.getHealth() + entity.getAbsorptionAmount()),
                        entity.getHeldItemOffhand().getItem() == Items.TOTEM_OF_UNDYING, XSWARE.POP_MANAGER.getTotemPops(entity)));
            }
        }
    }
    ...
    private void renderNameTag(String name, double x, double yi, double z, float delta, double xPos, double yPos, double zPos, double hp, boolean hasTotem, int pops) {
        ...
        int width = XSWARE.GUI_FONT_MANAGER.getTextWidth(displayTag) / 2;
        int width2 = XSWARE.GUI_FONT_MANAGER.getTextWidth(displayTag2) / 2;
        ...
        GlStateManager.enableBlend();
        RenderUtil.drawRect(-width2 - 2, -(XSWARE.GUI_FONT_MANAGER.getTextHeight() + 1), (float) width2 + 2.0f, 14f, 0x55000000);
        GlStateManager.disableBlend();
        XSWARE.GUI_FONT_MANAGER.drawStringWithShadow(displayTag, -width, -(XSWARE.GUI_FONT_MANAGER.getTextHeight() - 1), colour.getValue().getRGB());
        XSWARE.GUI_FONT_MANAGER.drawStringWithShadow(displayTag2, -width2, -(XSWARE.GUI_FONT_MANAGER.getTextHeight() - 1 - 12), colour.getValue().getRGB());
