...
import me.xss6.xsware.mixin.mixins.accessors.IChunkProviderClient;
...
@Module.Registration(name = "Chunk Scanner", description = "Finds coords thx to azrn", category = Module.Category.PLAYER)
        ...
        if(event.getPacket() instanceof SPacketBlockChange) {
            ...
            try {
                if(saveCoords.getValue()) {
                    XSWARE.CONFIG_MANAGER.saveCoords("X: " + x + " Z: " + z);
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
