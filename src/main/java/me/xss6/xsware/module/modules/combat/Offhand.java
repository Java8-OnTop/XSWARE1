...
import me.xss6.xsware.util.CrystalUtil;
...
@Module.Registration(name = "Offhand", description = "puts things in ur offhand", category = Module.Category.COMBAT, priority = ModulePriority.Highest)
public class Offhand extends Module {
    ...
    private boolean Check32K() {
        ...
        for (Entity entity : mc.world.loadedEntityList) {
            if (entity != mc.player && XSWARE.FRIEND_MANAGER.isFriend(entity.getName()) && entity instanceof EntityPlayer && entity.getDistance(mc.player) < 7) {
    ...
    public static class StopPlayerMovement {
        ...
        public static void toggle(boolean on) {
            if (on) {
                XSWARE.EVENT_PROCESSOR.addEventListener(stopPlayerMovement);
            } else {
                XSWARE.EVENT_PROCESSOR.removeEventListener(stopPlayerMovement);
            }
        }
