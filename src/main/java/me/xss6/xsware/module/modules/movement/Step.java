...
@Module.Registration(name = "Step", description = "steps up things", category = Module.Category.MOVEMENT)
public class Step extends Module {
    ...
    public void onUpdate() {
        if (nullCheck()) return;
        if (mc.player.onGround && !mc.player.isInsideOfMaterial(Material.WATER) && !mc.player.isInsideOfMaterial(Material.LAVA) && mc.player.collidedVertically && mc.player.fallDistance == 0.0f && !mc.gameSettings.keyBindJump.pressed && !mc.player.isOnLadder() && !XSWARE.Modules.ishackEnabled("Speed")) {
            if (!vanilla.getValue()) {
