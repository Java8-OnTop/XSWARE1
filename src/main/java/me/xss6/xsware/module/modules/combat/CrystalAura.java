package me.xss6.xsware.module.modules.combat;
...
import me.xss6.xsware.module.modules.misc.AutoClip;
import me.xss6.xsware.setting.type.*;
import me.xss6.xsware.util.*;
...
@Module.Registration(name = "Crystal Aura", description = "epic", category = Module.Category.COMBAT, priority = ModulePriority.Highest)
public final class CrystalAura extends Module {
    ...
    private final me.xss6.xsware.util.elements.Timer crystalsPlacedTimer = new Timer();
...
// silly timeout
    ...
    public EntityEnderCrystal getBestCrystal() {
        ...
        if (this.ezTarget != null) {
            XSWARE.KD_MANAGER.targets.put(this.ezTarget.getName(), 20);
    ...
    private boolean isPlayerValid(@NotNull EntityPlayer player) {
        ...
        if (XSWARE.FRIEND_MANAGER.isFriend(player.getName())) return false;
    ...
    private boolean setYawPitch(@NotNull EntityEnderCrystal crystal) {
        ...
        float yaw = angle[0];
        float pitch = angle[1];
        float spoofedYaw = XSWARE.ROTATION_MANAGER.getSpoofedYaw();
        if (Math.abs(spoofedYaw - yaw) > maxYaw.getValue() && Math.abs(spoofedYaw - 360 - yaw) > maxYaw.getValue() && Math.abs(spoofedYaw + 360 - yaw) > maxYaw.getValue()) {
            XSWARE.ROTATION_MANAGER.setPlayerRotations(Math.abs(spoofedYaw - yaw) < 180 ? spoofedYaw > yaw ? (spoofedYaw - maxYaw.getValue()) : (spoofedYaw + maxYaw.getValue()) : spoofedYaw > yaw ? (spoofedYaw + maxYaw.getValue()) : (spoofedYaw - maxYaw.getValue()), pitch);
            return false;
        } else {
            XSWARE.ROTATION_MANAGER.setPlayerRotations(yaw, pitch);
        }
        return true;
    }
