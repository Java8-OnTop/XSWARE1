...
import me.xss6.xsware.util.DamageUtil;
...
@Module.Registration(name = "Kill Aura", description = "hits people", category = Module.Category.COMBAT, priority = ModulePriority.Low)
public class KillAura extends Module {
    ...
    public void onUpdateWalkingPlayerEvent(UpdateWalkingPlayerEvent event) {
        if (event.getStage() == 0 && this.rotate.getValue()) {
            if ((this.target = this.getTarget()) != null) {
                XSWARE.ROTATION_MANAGER.lookAtEntity(this.target);
                this.doKA();
            }
        }
    }
    ...
    private float getLagComp() {
        return -(20 - XSWARE.SERVER_MANAGER.getTPS());
    }
