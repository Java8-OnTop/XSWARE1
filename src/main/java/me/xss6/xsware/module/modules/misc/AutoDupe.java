 ...
 * @since 10/07/2021
 */
@Module.Registration(name = "Auto Dupe", category = Module.Category.MISC, description = "Dupes for u", priority = ModulePriority.Low)
public final class AutoDupe extends Module {
    EnumSetting server = new EnumSetting("Server", "gcianarchy.xyz", Arrays.asList("gcianarchy.xyz"), this);
    EnumSetting modes = new EnumSetting("Mode", "Main", Arrays.asList("Main", "Slave"), this, s -> server.is("gcianarchy.xyz"));
    BooleanSetting sendChantMessage = new BooleanSetting("Chat Message", true, this, s -> server.is("gcianarchy.xyz"));
    BooleanSetting waitItems = new BooleanSetting("Wait For Items", true, this, s -> server.is("gcianarchy.xyz"));
    ...
    public void onUpdate(){
        ...
        if(server.is("gcianarchy.xyz") && modes.is("Slave")) {
        ...
        if(server.is("gcianarchy.xyz") && modes.is("Main")){
            BlockPos pPos = PlayerUtil.getPlayerPos();
            if(shouldWait){
                ...
                if(sendChantMessage.getValue()) {
                    mc.player.sendChatMessage("Auto duping thanks to XSWARE!");
                }
                return;
            }
            ...
            switch (stage){
                case 0:
                    ...
                    if(cSlot == -1){
                        ClientMessage.sendMessage("You do not have a chest in your hotbar!");
                        this.toggle();
                        return;
                    }
                ...
                case 1:
                    ...
                    if(tntSlot == -1){
                        ClientMessage.sendMessage("You do not have tnt in your hotbar!");
                        this.toggle();
                        return;
                    }
