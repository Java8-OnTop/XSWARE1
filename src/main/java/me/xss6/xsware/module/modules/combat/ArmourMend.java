...
@Module.Registration(name = "Auto Armour", description = "automates ur armour", category = Module.Category.COMBAT)
public class ArmourMend extends Module {
    ...
    private final me.xss6.xsware.util.elements.Timer timer = new Timer();
    ...
    public void onTick() {
        ...
        if (this.taskList.isEmpty()) {
            ...
            if (this.mendingTakeOff.getValue() && InventoryUtil.holdingItem(ItemExpBottle.class) &&
                    ...
                    mc.world.playerEntities.stream().noneMatch(e ->
                            e != mc.player && !XSWARE.FRIEND_MANAGER.isFriend(e.getName())
        ...
        if (this.timer.passedMs((int) ((float) this.delay.getValue() * XSWARE.SERVER_MANAGER.getTpsFactor()))) {
            if (!this.taskList.isEmpty()) {
