...
import me.xss6.xsware.event.events.JesusEvent;
...
@Module.Registration(name = "Jesus", description = "its jebus, say hello jebus SHGDFYGSDKJHFGSDHJ", category = Module.Category.MOVEMENT, isListening = false)
public class Jesus extends Module {
    ...
    public void updateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (nullCheck() || XSWARE.Modules.ishackEnabled("Freecam")) {
            return;
        }
    ...
    public void sendPacket(PacketEvent.Send event) {
        if (event.getPacket() instanceof CPacketPlayer && XSWARE.Modules.ishackEnabled("Freecam")
