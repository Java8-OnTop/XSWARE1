...
public class FriendCommand extends Command {
    ...
    public void execute(String[] message) {
        if (message.length == 0) {
            ClientMessage.sendErrorMessage("Need more info than that idiot");
            return;
        }
        if (message.length == 1) {
            if (message[0].equalsIgnoreCase("list")) {
                if (XSWARE.FRIEND_MANAGER.hasFriends()) {
                    ClientMessage.sendMessage(ChatFormatting.BOLD + "Listing friends");
                    for (XswarePlayer player : XSWARE.FRIEND_MANAGER.getFriends()) {
