...
public class CategoryComponent implements Globals {
    ...
    public CategoryComponent(Hack.Category cat) {
        ...
        this.height = XswareGuiNew.HEIGHT;
        this.x = 5;
        this.y = 5;
        this.dragX = 0;
        this.isOpen = true;
    ...
    public void renderFrame(int mouseX, int mouseY) {
        ...
        if (Gui.INSTANCE.customFont.getValue()) {
            XSWARE.GUI_FONT_MANAGER.drawStringWithShadow(category.getName(), this.x + XswareGuiNew.MODULE_FONT_SIZE, this.y + (this.height / 2f) - XswareGuiNew.FONT_HEIGHT, Gui.INSTANCE.fontColor.getValue().hashCode());
        } else {
            mc.fontRenderer.drawStringWithShadow(category.getName(), this.x + XswareGuiNew.MODULE_FONT_SIZE, this.y + (this.height / 2f) - XswareGuiNew.FONT_HEIGHT, Gui.INSTANCE.fontColor.getValue().hashCode());
        }
        if (this.isOpen) {
            if (!this.components.isEmpty()) {
                ...
                x *= XswareGuiNew.HEIGHT + XswareGuiNew.MODULE_OFFSET;
                switch (Gui.INSTANCE.type.getValue()) {
                    case "Sin":
                        ...
                        RenderUtil2D.drawVLineG(this.x + 4, this.y + 1 + XswareGuiNew.HEIGHT - 1, x,
                    ...
                    case "Rainbow":
                        ...
                        break;
                    case "None":
                        RenderUtil2D.drawVLine(this.x + 4, this.y + 1 + XswareGuiNew.HEIGHT - 1, x, Gui.INSTANCE.buttonColor.getValue().hashCode());
                        break;
                }
            }
        }
