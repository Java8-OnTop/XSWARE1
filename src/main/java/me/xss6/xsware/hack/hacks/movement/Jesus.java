...
@Hack.Registration(name = "Jesus", description = "its jebus, say hello jebus SHGDFYGSDKJHFGSDHJ", category = Hack.Category.MOVEMENT, isListening = false)
public class Jesus extends Hack {
    ...
    public void updateWalkingPlayer(UpdateWalkingPlayerEvent event) {
        if (nullCheck() || XSWARE.HACKS.ishackEnabled("Freecam")) {
            return;
        }
    ...
    public void sendPacket(PacketEvent.Send event) {
        if (event.getPacket() instanceof CPacketPlayer && XSWARE.HACKS.ishackEnabled("Freecam")
