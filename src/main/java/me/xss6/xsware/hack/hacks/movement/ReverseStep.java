...
@Hack.Registration(name = "Reverse Step", description = "pulls u down down down", category = Hack.Category.MOVEMENT)
public class ReverseStep extends Hack {
    ...
    public void onUpdate() {
        if (nullCheck()) return;
        try {
            if (mc.player.isInLava() || mc.player.isInWater() || mc.player.isOnLadder() || XSWARE.HACKS.ishackEnabled("Speed")) return;
