...
public class Loader {
    ...
    public Loader() {
        XSWARE.LOGGER.info("Loading Plugins");
        pluginDir = new File(XSWARE.CONFIG_MANAGER.pluginFolder);
    ...
    public void loadClasses() {
        if (pluginsFiles != null && pluginsFiles.length > 0) {
            ...
            classes.spliterator().forEachRemaining(name -> {
                try {
                    ...
                    XSWARE.LOGGER.info(ife[0].getName());
                    if (ife[0].equals(Plugin.class)) {
                        XSWARE.LOGGER.info("asd");
        ...
        if (!plugins.isEmpty()) {
            XSWARE.LOGGER.info(plugins.get(0).name());
            plugins.spliterator().forEachRemaining(Plugin::init);
        }
        if(!names.isEmpty()) {
            for (String name : names) {
                XSWARE.LOGGER.info("Loaded -> " + name);
            }
        }
        XSWARE.LOGGER.info("Loaded Plugins");
    }
