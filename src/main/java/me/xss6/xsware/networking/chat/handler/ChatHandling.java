...
import me.xss6.xsware.networking.chat.packets.ping.PingGetGlobal;
    ...
    public void chatLoop(){
        while (running) {
            try {
                if (mode.equals(ChatMode.GLOBAL)) {
                    if (System.currentTimeMillis() - lastPing == 1000) {
                        try {
                            ...
                            String[] data = getChatPacket.run(XSWARE.CLIENT_HANDLING.token);
                ...
                XSWARE.LOGGER.error("Exception in ChatLoop for IRC " + e);
            }
        }
    }
